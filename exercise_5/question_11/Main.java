package exercise_5.question_11;
...
public class Main {
        ...
        while (!correct) {
            switch (input) {
                ...
                case "4" -> {
                    System.out.println("Enter account username: ");
                    String account_username = scanner.nextLine();
                    addRandomGroup(account_username, groups);
                    System.out.println("Bạn có muốn nhập chức năng khác không?");
                    String answer = scanner.nextLine();
                    if(answer.equals("Không")) {
                        correct = true;
    ...
    private static void addRandomGroup(String username, List<Group> groupList) {
        Random random = new Random();
        int randomIndex = random.nextInt(groupList.size());
        Group randomGroup = groupList.get(randomIndex);
        randomGroup.getAccount_name().add(username);
    }
